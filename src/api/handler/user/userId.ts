 ...
 */
export const getUsersUserId = rest.get<never, { userId: string }, UserType>(
  endpoint,
  (req, res, ctx) => {
    ...
    return res(
      ctx.delay(1000),
      ctx.status(200),
      ctx.json({ ...EXAMPLE_USER, id: userId })
    );
  }
);
 ...
 */
export const putUsersUserId = rest.put<string, { userId: string }, UserType>(
  endpoint,
  (req, res, ctx) => {
